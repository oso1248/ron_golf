...
async function hole_update(ev) {
  ...
  let response = await upload_hole(send_data);
  alert(response);
}
...
function shape_data(data) {
  ...
  data.forEach((elem) => {
    ...
    let string_elem = `(${elem.id}, ${par}, ${distance},${handicap}),`;
...
async function upload_hole(data) {
  try {
    let res = await axios.post('/api/admin/hole_update_name', data);
    if (res.data.details[0].message) {
      ...
      return `${res.data.details[0].count} Rows Have Been Updated`;
    }
  } catch (err) {
    return err;
  }
